module UI.SortableTable exposing
    ( view
...
view (Config { toId, toMsg, columns, customizations }) state data =
        ...
        withFoot =
            case customizations.tfoot of
                ...
                Just { attributes, children } ->
                    tfoot attributes children :: tbody :: []
    in
    table customizations.tableAttrs <|
        case customizations.caption of
            ...
            Just { attributes, children } ->
                caption attributes children :: thead_ :: withFoot
...
viewRowHelp columns toRowAttrs data =
    tr (toRowAttrs data) (List.map (viewCell data) columns)
...
viewCell data { viewData } =
    let
        details =
            viewData data
    in
    td details.attributes details.children
